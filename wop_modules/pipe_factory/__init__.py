...
from wop_modules.copyacut_processor import CopyacutProcessor, CopyacutProcessorConfig
...
class ConfigParser(object):
    ...
    def parser(self) -> ConfigParserReturn:
        ...
        for task in [t for t in tasks if isinstance(t.action, ConfigActions)]:
    ...
    def map_task_to_processor(task: TaskShaped) -> Callable:
        ...
        # shell命令行的处理
        if action == ConfigActions.SHELL:
            shell_config = task.shell_config
            return lambda: ShellProcessor(shell_config.cwd).execute(shell_config.cmd)
        # copy和cut的处理
        elif action == ConfigActions.COPY_AND_CUT:
            copy_and_cut_config = task.copyacut_config
            return lambda: CopyacutProcessor(
                CopyacutProcessorConfig(
                    action=copy_and_cut_config.action, from_path=copy_and_cut_config.fro,
                    to_path=copy_and_cut_config.to, override=copy_and_cut_config.override
                )
            ).execute_copy_and_cut()
        # zip处理
        elif action == ConfigActions.ZIP:
